                    ...
                    <ol class="breadcrumb">
                        ...
                        @foreach($pathComponents as $index => $component)
                            ...
                            @php if($i>2){
                                ...
                                @endphp
                                <li class="breadcrumb-item">
                                    <a href="{{ route('folderContent', ['id' => $id, 'name' => $component,'parent'=>$parent]) }}"><img  width="30" @if($id==$folderId) src="{{ URL::asset('img/open-folder.png')}}"   @else src="{{ URL::asset('img/folder.png')}}" @endif > {{ $component }}</a>
                                </li>
                            @php } @endphp
                        @endforeach
                        @if($files)
