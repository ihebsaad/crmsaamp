...
*/
	...
	{ 
		...
		// Diviser les rendez-vous en deux catégories: clientèles et hors clientèles
		$rendezVousClienteles = $rendezvous->where('AccountId', '>', 0);
		$rendezVousHorsClienteles = $rendezvous->where('AccountId', '==', 0);
		
		// Regrouper les rendez-vous par type dans chaque catégorie
		$rendezVousClientelesByType = $rendezVousClienteles->groupBy('Type');
		$rendezVousHorsClientelesByType = $rendezVousHorsClienteles->groupBy('Type');
		 
		$date = date('d_m_Y_H_i'); 
		$pdf = PDF::loadView('rendezvous.pdf_synthese', compact(
			'rendezVousClientelesByType', 
			'rendezVousHorsClientelesByType', 
			'user', 
			'name', 
			'date_debut', 
			'date_fin'
